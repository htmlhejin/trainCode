<template>
  <div class="goods" v-if="shopInfo">
        ...
        <ul>
          <li v-for="(item,index) in recommend.items" :key="index">
            ...
            <div class="recommend-food">
              ...
              <p class="recommend-zm">月售{{item.month_sales}} 好评率{{item.satisfy_rate}}</p>
            </div>
    ...
    <ShopCart :shopInfo="shopInfo"></ShopCart>
     <!--商品详情  -->
    <Food :isShow="showFood" :food="selectedFood" @close="showFood=false"></Food>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      listHeight: [], //12个列表区域高度
      selectedFood:null,
      showFood:false
    };
  },
  // 通过计算属性计算currentIndex的值
  computed: {
    // 根据右侧滚动位置，确定对应的索引下标
    currentIndex() {
  ...
  methods: {
    ...
    initScroll() {
      ...
      this.foodScroll.on("scroll", pos => {
        // console.log(pos.y)   //看一下右侧food的y值  -1128.96   -1128.99
        this.scrollY = Math.abs(Math.round(pos.y)); //Math.round 四舍五入得到最接近的整数。abs取绝对值
      });
    },
    // 点击左侧menu跳到右边对应的food
    selectMenu(index) {
      // 通过ClassName拿到所有的food
      let foodlist = this.$refs.foodScroll.getElementsByClassName(
        "food-list-hook"
      );
      let el = foodlist[index];
      // console.log(el)   //打印除所有li
      this.foodScroll.scrollToElement(el, 260); //滚动时间260ms
    },
    // 计算12个区的高度
    calculateHeight() {
      let foodlist = this.$refs.foodScroll.getElementsByClassName(
